...
with img_col:
    ...
    if has_images:
        include_images = option_row("Images and masks", False, "dl_include_images", "The image files and their segmentation mask files.", on_change=_invalidate)
        option_row("Colored mask overlays", False, "dl_include_overlay", "Color-coded regions drawn over each image to visualize the segmented cells.", disabled=not include_images, on_change=_invalidate)
...
with table_col:
    ...
    if has_images:
        option_row("Per-image cell counts", False, "dl_include_summary", "CSV listing how many cells of each class appear in each image.", on_change=_invalidate)
        option_row("Cell metrics", False, "dl_include_cell_metrics", f"CSV of {len(METRIC_COLS)} morphological descriptors ({', '.join(METRIC_COLS[:3])}, etc.) for every cell.", on_change=_invalidate)
    else:
        st.caption("No data available yet.")

with model_col:
    st.subheader("Trained Models", text_alignment="center")
    option_row("Cellpose model", False, "dl_include_cellpose", "Fine-tuned model weights, the training dataset, and loss curves.", disabled=not has_cellpose, on_change=_invalidate)
    option_row("DenseNet model", False, "dl_include_densenet", "Fine-tuned model weights, the training dataset, and evaluation metrics.", disabled=not has_densenet, on_change=_invalidate)
